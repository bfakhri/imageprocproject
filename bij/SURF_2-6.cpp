...
int main() {
	...
	imwrite("./output/26_img2_surf_kps.jpg", draw_img2_surf_kps); 
	namedWindow("IM6 SURF KeyPoints", WINDOW_AUTOSIZE);
	imshow("IM6 SURF KeyPoints", draw_img6_surf_kps);
	imwrite("./output/26_img6_surf_kps.jpg", draw_img6_surf_kps); 
	cv::waitKey(0);
	...
	imwrite("./output/26_surf_matches.jpg", draw_surf_matches); 
	namedWindow("Best SURF Matches", WINDOW_AUTOSIZE);
	imshow("Best SURF Matches", draw_best_surf_matches);
	imwrite("./output/26_best_surf_matches.jpg", draw_best_surf_matches); 
	cv::waitKey(0);
